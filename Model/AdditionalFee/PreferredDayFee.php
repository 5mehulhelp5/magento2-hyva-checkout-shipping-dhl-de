...
class PreferredDayFee implements AdditionalFeeProviderInterface
{
    ...
    public function getFee(Quote $quote): float
    {
        ...
        foreach ($selections as $selection) {
            // Prüft, ob die Option "Preferred Day" aktiviert ist
            if ($selection->getShippingOptionCode() === Codes::SERVICE_OPTION_PREFERRED_DAY && !empty($selection->getInputValue())) {
                // Holt den Gebührenwert aus der Konfiguration
                $feeConfigPath = 'carriers/dhlpaket/service_preferred_day_charge';
